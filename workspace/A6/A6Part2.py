...
def segmentStableNotesRegions(inputFile = '../../sounds/sax-phrase-short.wav', stdThsld=10, minNoteDur=0.1, 
    ...
    i = 0
    for value in stable:
        if stable[min(i+1, stable.size - 1)] > value + 1:
    ...
    for segment in segments:
        if (fs * minNoteDur) < (segment[1] - segment[0]) * H:
           filtered.append(segment)

    results = np.array(filtered)
    # plotSpectogramF0Segments(x, fs, w, N, H, f0, results)  # Plot spectrogram and F0 if needed
